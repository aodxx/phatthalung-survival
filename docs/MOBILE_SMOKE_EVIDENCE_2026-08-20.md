...
After 99fd3a9, a controlled TEST flow also forced a queued row to an old `UPLOADING` lock and dispatched `online`. The browser still timed out without a post-offline upload request; this confirms the stale-lock recovery code is covered by unit tests but the Playwright network-control environment does not reliably emit/execute the reconnect transition. The browser drain item remains CONDITIONAL; local TEST IndexedDB was cleared afterward.
